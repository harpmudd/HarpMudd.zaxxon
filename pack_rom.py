...
# MiSTer Arcade-Zaxxon .mra (carry the digitized samples). Same resolution: the
# HARPMUDD_MRA env var, the .mra next to this script, or the MiSTer releases dir
# beside the repo.
MRA_DIR = next(
    (d for d in (os.environ.get("HARPMUDD_MRA"),
                 os.path.join(os.path.dirname(HERE), "Downloaded_Artifacts",
                              "Arcade-Zaxxon_MiSTer-master", "releases"))
     if d and os.path.isdir(d)),
    HERE)
