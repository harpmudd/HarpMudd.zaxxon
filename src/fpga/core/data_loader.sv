...
// Ported from opengateware/arcade-galaga modules/dataloader-pocket/data_loader.sv
//
// A data loader for consuming APF bridge writes and directing them to some storage medium.
// Takes 32-bit words from APF and splits them into byte (or 16-bit) writes synchronous
// to clk_memory, using a dcfifo to safely cross clock domains.

module data_loader #(
    // Upper 4 bits of address (bridge_addr[31:28]) that this loader will respond to
    parameter ADDRESS_MASK_UPPER_4 = 0,
    parameter ADDRESS_SIZE = 14,

    // Number of clk_memory cycles to delay each write output. Min 4.
    // Note: APF sends data every ~75 clk_74a cycles, so memory delay must keep up.
    parameter WRITE_MEM_CLOCK_DELAY = 4,

    // Number of clk_memory cycles to hold write_en high. Min 1.
    parameter WRITE_MEM_EN_CYCLE_LENGTH = 1,

    // Word size in bytes: 1 = 8-bit output, 2 = 16-bit output.
    ...
    // Outputs are synchronous to clk_memory
  ...
  // ===== APF -> Memory clock (clk_74a domain) =====
  ...
  always @(posedge clk_74a) begin
    ...
    case (write_state)
      ...
      WRITE_REQ_SHIFT: begin
        ...
        if (write_count == (4 / OUTPUT_WORD_SIZE) - 1)
          write_state <= 0;
        else
  ...
  // ===== Memory clock -> core writes (clk_memory domain) =====
